...
<div class="modal-body scroll-y px-10 px-lg-15 pt-0 pb-15">
    <form id="form_edit_admin" class="form">
        <input type="hidden" class="form-control form-control-solid" id="admin_id" name="admin_id" value="<?php echo $row['admin_id'] ?>" />

        <div class="mb-13 text-center">
            <h1 class="mb-3">แก้ไขผู้ดูแลระบบ</h1>
            <div class="text-muted fw-bold fs-5">( <?php echo $row['fullname'] ?> )
            </div>
        </div>

        <div class="d-flex justify-content-center align-items-center mb-5 fv-row">
        ...
        <div class="d-flex flex-column mb-8 fv-row">
            ...
            <select class="form-select form-select-solid" id="admin_status" name="admin_status" data-control="select2" data-dropdown-parent="#modal" data-placeholder="กรุณากำหนดสิทธิ์" data-allow-clear="true">
                <option></option>
                <option value="1" <?php if ($row['admin_status'] == '1') {
                                        echo 'selected';
                                    } ?>>ผู้ดูแลระบบ</option>
                <option value="2" <?php if ($row['admin_status'] == '2') {
                                        echo 'selected';
                                    } ?>>ผู้ดูข้อมูลเท่านั้น</option>
                <option value="3" <?php if ($row['admin_status'] == '3') {
                                        echo 'selected';
                                    } ?>>ผู้เพิ่มพนักงานเท่านั้น</option>
            </select>
        </div>
        <!--begin::Actions-->
        <div class="text-center">
            <button type="button" class="btn btn-sm btn-light btn-hover-scale me-3" data-bs-dismiss="modal">ปิด</button>
            <button type="button" id="modal_admin_submit" class="btn btn-sm btn-primary btn-hover-scale" onclick="Edit();">
