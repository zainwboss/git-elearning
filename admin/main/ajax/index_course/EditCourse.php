...
if (!empty($_SESSION['admin_id'])) {
    ...
    //course_group
    $sql_delete = "DELETE FROM tbl_course_group WHERE course_id ='$course_id'";
    ...
    if ($_FILES['main_image'] != "") {
        ...
        if (in_array($file_surname, array('jpeg', 'jpg', 'png', 'JPEG', 'JPG', 'PNG'))) {
            if (move_uploaded_file($_FILES['main_image']['tmp_name'], "../../../../images/" . $filename_images)) {
                $sql1 = "SELECT main_image FROM tbl_course WHERE course_id='$course_id'";
                $rs1 = mysqli_query($connection, $sql1);
                $row1 = mysqli_fetch_array($rs1);
                unlink('../../../../images/' . $row1['main_image']);
